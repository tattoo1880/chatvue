<template>
    ...
    <div>
        <el-row class='row-bg' justify='end'>
            <el-col :span='2'>
                <el-button type='success' plain @click='addcustomer'>添加客户</el-button>
            </el-col>
        </el-row>
    </div>
        ...
        <el-row class="row-bg" justify="center" style="margin-top: 30px;">
            <el-col :span="24">
                <el-table :data="tabledata" style="width: 100%" align="center">
                    ...
                    <el-table-column prop="name" label="用户名" width="150" show-overflow-tooltip align="center">
                        <template v-slot="{ row }">
    ...
    <!-- 添加客户的dialog -->
    <div>
        <el-dialog v-model="showme1" title="添加用户" width="500" :before-close="handleClose">
            <div>
    ...
    <div>
        <el-dialog v-model="showme2" title="添加用户" width="500" :before-close="handleClose2">
            <div>
                <el-text class='mx-1' type='success' size='large'>
                    更新用户
                </el-text>
...
const handleConfirm = async () => {
    ...
    try {
        const res = await myfetch.post("user/", customerdetail.value)
        console.log(res.data)
...
const handerdelete = async (id) => {
    console.log(id)
    try {
        const res = await myfetch.delete(`user/${id}/`)
        console.log(res.data)
...
const handleConfirm2 = async () => {
    ...
    try {
        const res = await myfetch.put(`user/${updatacustomer.value.uuid}/`, updatacustomer.value)
        console.log(res.data)
        if (res.data) {
            showme2.value = false
        }
